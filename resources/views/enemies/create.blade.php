<x-layout title="New Enemy">
  <section class="w-full">
    <figure class="thumbnail">
      <img src="/img/enemies.jpg" alt="enemies" class="thumbnail-img">
      <h1 class="thumbnail-title">Create an Enemy</h1>
    </figure>
  <form class="form-default mt-4" action="{{ route('enemies.store') }}" method="POST" enctype="multipart/form-data">
      @csrf
      ...
      <div class="form-group">
        ...
        <ul class="w-full flex flex-col items-start md:flex-row md:gap-4">
          @foreach ($areas as $area)
            <li class="text-lg font-bold"><input class="mr-1 cursor-pointer" type="checkbox" value="{{ $area->id }}" name="areas[]">{{ $area->name }}</li>
          @endforeach
        </ul>
      </div>
